...
function build_profit_model(data_file::String)
    ...
    @constraint(m, MaxPetrodisel, 
        sum(Petro_disel_used[i] * b[i] for i in I_fuel_mixtures) <= Max_amount_petrodisel)
    @constraint(m, MaxVegOil, 
        sum(Veg_oil_used[i] * b[i] for i in I_fuel_mixtures) <= Max_amount_vegetable_oil)
    @objective(m, Max, sum(Profit_fuel_mixtures[i] * b[i] for i in I_fuel_mixtures))
